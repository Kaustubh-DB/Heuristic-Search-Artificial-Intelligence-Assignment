...
LUDDY_MOVES = {"A": (-2, -1), "B": (-2, 1), "C": (2, -1), "D": (2, 1), "E": (-1, -2), "F": (-1, 2), "G": (1, -2),
               "H": (1, 2)}
...
def solve(initial_board):
    ...
    q.put(((h_cost + g_cost), initial_board, ""))

    while not q.empty():
        (f, state, route_so_far) = q.get()
        closed.append(state)
        if is_goal(state):
            return route_so_far
        ...
        for (succ, move) in successors(state):
            ...
            q.put(((h_cost + g_cost), succ, route_so_far + move))
            g_cost += 1
